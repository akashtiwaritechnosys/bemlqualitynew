<?php /* Smarty version Smarty-3.1.19, created on 2024-12-11 11:38:14
         compiled from "/var/www/html/bemlqualitynew/customerportal/layouts/default/templates/Header.tpl" */ ?>
<?php /*%%SmartyHeaderCode:134371842167592c4ea4b643-06736079%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '1c764b85f959aae235a044f519ad8225efe102be' => 
    array (
      0 => '/var/www/html/bemlqualitynew/customerportal/layouts/default/templates/Header.tpl',
      1 => 1733391280,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '134371842167592c4ea4b643-06736079',
  'function' => 
  array (
  ),
  'variables' => 
  ...
  'unifunc' => 'content_67592c4ea859a3_59491920',
),false); /*/%%SmartyHeaderCode%%*/?>
<?php if ($_valid && !is_callable('content_67592c4ea859a3_59491920')) {function content_67592c4ea859a3_59491920($_smarty_tpl) {?>
...
"></script>
		...
		<link rel="stylesheet" href="layouts/<?php echo portal_layout();?>
/skins/default/stylechanges.css" type="text/css" media="screen" />
	...
	<body ng-controller="MainController" ng-cloak>
		<nav class="navbar navbar-inverse navbar-static-top cp_header">
			...
			<div class="collapse navbar-collapse" role="navigation">
				...
				<div class="col-lg-10 col-md-10 col-sm-10 col-xs-12 portalnavbar customerportal_menu" ng-if="loginUser">
					<ul class=" nav navbar-nav headerLinks menu" ng-show="modules">
						...
						<li class="hidden-md hidden-sm hidden-lg" ng-repeat="module in modules|orderBy:'order'" ng-if="module!=='language' && module.name!=='ProjectTask' && module.name!=='CallAssistance' && module.name!=='DeliveryNotes'" ng-class="{'active':isActive(module.name)}">
							<a translate="{{module.uiLabel}}" href="index.php?module={{module.name}}">
                                    {{module.uiLabel}}
                                </a>
						</li>
						<li class="hidden-xs hidden-sm" ng-repeat="module in modules|orderModulesBy:'order'" ng-if="module!=='language' && module.name!=='ProjectTask' && module.name!=='CallAssistance' && module.name!=='DeliveryNotes'" ng-class="{'active':isActive(module.name)}">
							<a ng-if="$index <=1" translate="{{module.uiLabel}}" href="index.php?module={{module.name}} ">{{module.uiLabel}}</a>
						</li>
						<li class="hidden-xs hidden-sm hidden-md visible-lg" ng-repeat="module in modules|orderModulesBy:'order'" ng-if="module!=='language' && module.name!=='ProjectTask' && module.name!=='CallAssistance' && module.name!=='DeliveryNotes'" ng-class="{'active':isActive(module.name)}">
						...
						<li class="hidden-xs navbartoggle" ng-if="modulesCount > 5">
							...
							<ul class="dropdown-menu">
								<li class="hidden-md hidden-lg" ng-repeat="module in modules|orderModulesBy:'order'" ng-if="module!=='language' && module.uiLabel!==undefined && module.name!=='ProjectTask' && module.name!=='DeliveryNotes'" ng-class="{'active':isActive(module.name)}">
									<a ng-if="$index>-1 && $index<=1" translate="{{module.uiLabel}}" href="index.php?module={{module.name}}">{{module.uiLabel}}</a>
								</li>
								<li class="hidden-lg" ng-repeat="module in modules|orderModulesBy:'order'" ng-if="module!=='language' && module.name!=='ProjectTask' && module.name!=='CallAssistance' && module.name!=='DeliveryNotes'" ng-class="{'active':isActive(module.name)}">
									<a ng-if="$index >1  && $index <=4" translate="{{module.uiLabel}}" href="index.php?module={{module.name}}">{{module.uiLabel}}</a>
								</li>
								<li ng-repeat="module in modules|orderModulesBy:'order'" ng-if="module!=='language' && module.name!=='ProjectTask' && module.name!=='CallAssistance' && module.name!=='DeliveryNotes'" ng-class="{'active':isActive(module.name)}">
					...
					<div class="col-lg-2 col-md-2 col-sm-2 col-xs-12 portalnavbar cp_login" ng-if="loginUser">
						<ul class="nav navbar-nav navbar-right headerLinks">
							<li class="dropdown">
								<a href="#" class="dropdown-toggle cp_login_user" data-toggle="dropdown">
