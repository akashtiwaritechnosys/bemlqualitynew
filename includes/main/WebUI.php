...
class Vtiger_WebUI extends Vtiger_EntryPoint {
	...
	function getLogin() {
		...
		if (!$user && isset($_SESSION['authenticated_user_id'])) {
			...
			// $fingerprint = hash('sha256', $_REQUEST['fingerprint'] . $_SERVER['HTTP_USER_AGENT'] . $_SERVER['REMOTE_ADDR'] . ($_SERVER['HTTP_ACCEPT_LANGUAGE'] ?? ''));
			// if (isset($_REQUEST['fingerprint']) && !empty($_REQUEST['fingerprint'])) {
			// 	if ($_SESSION['fingerprint'] !== $fingerprint) {
			// 		session_unset();
			// 		session_destroy();
			// 		header('Location: index.php?module=Users&parent=Settings&view=Login&error=sessionHijackmsg');
			// 		exit;
			// 	}
			// }
			...
			// 	session_unset();
			// 	session_destroy();
			// 	header ('Location: index.php?module=Users&parent=Settings&view=Login&error=sessionExipired');
			// 	exit;
			// }
			// $_SESSION['LAST_ACTIVITY'] = time();
			// $absolute_timeout = 3600; // 1 hour
			// if (time() - $_SESSION['CREATED'] > $absolute_timeout) {
			// 	session_unset();
			// 	session_destroy();
			// 	header ('Location: index.php?module=Users&parent=Settings&view=Login&error=sessionLifeExipired');
			// 	exit;
			// }

			// global $adb;
			// $currentSessionId = $_SESSION['session_id'];
			// $querRes = $adb->pquery("SELECT session_name FROM vtiger_users WHERE id = ?", [$userid]);
			// $data = $adb->fetchByAssoc($querRes, 0);
			// $storedSessionId = $data['session_name'];
			// if ($currentSessionId !== $storedSessionId) {
			// 	session_unset();
			// 	session_destroy();
			// 	header("Location: index.php?module=Users&parent=Settings&view=Login&error=concurrent");
			// 	exit;
			// }
		}
		return $user;
			...
			if(empty($module)) {
				if ($this->hasLogin()) {
					...
					if(!empty($defaultModule) && $defaultModule != 'Home' && $moduleModel && $moduleModel->isActive()) {
						...
						global $log;
						$fingerprint = hash('sha256', $_REQUEST['fingerprint'] . $_SERVER['HTTP_USER_AGENT'] . $_SERVER['REMOTE_ADDR'] . ($_SERVER['HTTP_ACCEPT_LANGUAGE'] ?? ''));
						$log->debug(" session finger print (".$_SESSION['fingerprint'].") method ...");
						$log->debug(" session finger print (".$fingerprint.") method ...");
						// if (isset($_REQUEST['fingerprint']) && !empty($_REQUEST['fingerprint'])) {
						// 	if ($_SESSION['fingerprint'] !== $fingerprint) {
						// 		session_unset();
						// 		session_destroy();
						// 		header('Location: index.php?module=Users&parent=Settings&view=Login&error=sessionHijackmsgTRIO');
						// 		exit;
						// 	}
						// } else {
						// 	session_unset();
						// 	session_destroy();
						// 	header('Location: index.php?module=Users&parent=Settings&view=Login&error=fingerprintdoestexists');
						// 	exit;
						// }
					}
				} else {
