...
function HandleTicketAssignment($entityData) {
    ...
    $equip_status = $recordInfo['equip_status'];
    $query = "UPDATE vtiger_equipment SET last_sr_status=? WHERE equipmentid=?";
    $adb->pquery($query, array($equip_status, $vtquipment_id));
    ...
    if (
        ...
        $recordInfo['ticket_type'] ==  'INSTALLATION OF SUB ASSEMBLY FITMENT'
    ) {
    ...
    // $query = "INSERT INTO `vtiger_ticket_mttr` (`id`, `from_val`, `to_val`,
    //  `minutes_val`) VALUES (?,?,?,?,?);";
    // $adb->pquery($query, array($id, 'Open', 'Open', 0));
}
