...
class HelpDesk_Detail_View extends Vtiger_Detail_View {
	...
	public function showModuleDetailView(Vtiger_Request $request) {
		...
		$viewer->assign('STATUS_CHANGE_HISTORY', $this->getStatusChangeHistory($recordId));
	...
	function preProcess(Vtiger_Request $request, $display = true) {
		...
		$ticketAcceptanceStatus = $recordModel->get('tket_acc_status');
		$ticketRejectionReason = $recordModel->get('rejection_reason');
		if($ticketAcceptanceStatus == 'Rejected' && $ticketRejectionReason != 'Not a valid Service Request' 
		&& $ticketRejectionReason != 'Assistance provided to customer over call and rectified'){
			$viewer->assign('isEditableFromTicketFlow', 'true');
		} else {
			$viewer->assign('isEditableFromTicketFlow', 'false');
		}
		if ($display) {
	...
	public function getStatusChangeHistory($recordId) {
		global $adb;
		$sql = 'SELECT old_status,new_status,changed_at,duration_seconds
		FROM `status_history` where entity_id = ?';
		$result = $adb->pquery($sql, array($recordId));
		$records = [];
		array_push($records, array(
			'old_status' => 'Old Status',
			'new_status' => 'New Status',
			'changed_at_label' => 'Changed At',
