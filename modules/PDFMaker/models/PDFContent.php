...
class PDFMaker_PDFContent_Model extends PDFMaker_PDFContentUtils_Model {
    ...
    private function getTemplateData(){
        ...
        $result = self::$db->pquery("SELECT vtiger_pdfmaker.*, vtiger_pdfmaker_settings.* FROM vtiger_pdfmaker "
                ...
                array(self::$module,'Invoice','Quotes','SalesOrder','PurchaseOrder', 'ServiceReports','RecommissioningReports', self::$tempid));
...
//                            $breakline_type.= "</table>" . $tableTag["tag"];
    ...
    private function replaceFieldsToContent($emodule, $efocus, $is_related = false, $inventory_currency = false,$related = "R_"){
        ...
        $designationValue = self::$focus->column_fields["sr_designaion"];
        $changeRequiredDesignations = [
            "Assistant Engineer",
            "Senior Supervisor-S-6",
            "Senior Supervisor-S-5",
            "Senior Supervisor-S-4",
            "Supervisor- S-3",
            "Joint Supervisior-S-2",
            "Deputy Supervisor-S-1",
            "Master Skilled Technician-Gr.-E",
            "High Skilled Technician-Gr.-D",
            "Senior Technician-Gr.-C",
            "Technician-Gr.-B",
            "Helper- Gr-A",
        ];
        if (in_array($designationValue, $changeRequiredDesignations)) {
            self::$rep[strtoupper("$" . "SERVICEREPORTS" . "_" . "SR_DESI$")] = "Service Engineer";
        } else {
            self::$rep[strtoupper("$" . "SERVICEREPORTS" . "_" . "SR_DESI$")] = $designationValue;
        }
    ...
    private function convertEntityImages(){
        ...
        switch (self::$module){
            ...
            case "ServiceReports":
                ...
                $images = array();
                $images = $this->getHelpDeskImgAction(self::$focus->id, $site_URL_NonHttp,"customer_sign");
                
                self::$rep["$"."SERVICEREPORTS_CUSTOMER_SIGN"."$"] = $images;
                $images = array();
                $images = $this->getHelpDeskImgAction(self::$focus->id,$site_URL_NonHttp ,"service_eng_sign");
                self::$rep["$"."SERVICE_ENG_SIGN"."$"] = $images;

                $images = array();
                $images = $this->getHelpDeskImgActionAllImages(self::$focus->id, $site_URL_NonHttp,"vis_chk_ext_dam_img", 'External Damages');
                self::$rep["$"."VIS_CHK_EXT_DAM_IMG"."$"] = $images;

                $images = array();
                $images = $this->getHelpDeskImgActionAllImages(self::$focus->id, $site_URL_NonHttp,"vis_hyd_air_dam_img", 'Hydraulic & Air Leakages');
                self::$rep["$"."VIS_HYD_AIR_DAM_IMG"."$"] = $images;

                $images = array();
                $images = $this->getHelpDeskImgActionAllImages(self::$focus->id, $site_URL_NonHttp,"vis_lub_los_img", 'Lubrication');
                self::$rep["$"."VIS_LUB_LOS_IMG"."$"] = $images;

                $images = array();
                $images = $this->getHelpDeskImgActionAllImages(self::$focus->id, $site_URL_NonHttp,"vis_chk_oil_levels", 'Oil Levels');
                self::$rep["$"."VIS_CHK_OIL_LEVELS"."$"] = $images;

                $images = array();
                $images = $this->getHelpDeskImgActionAllImages(self::$focus->id, $site_URL_NonHttp,"vis_hyd_wrk_los_img", 'Work loosening of Hardwares');
                self::$rep["$"."VIS_HYD_WRK_LOS_IMG"."$"] = $images;
                break;
            case "Contacts":
    ...
    public function getHelpDeskImgAction($id, $site_url, $action) {
        $imagevalues = array();
        if (isset($id) and $id != "") {
            $db = PearDatabase::getInstance();
            $query = $this->getHelpDeskImgActionQuery($action);
            $result = $db->pquery($query, array($id));
            $num_rows = $db->num_rows($result);
            if($num_rows>0){
                if($action == "img_before_action"){
                    $images_html = "";
                }else{
                    $images_html = "";
                }
            ...
            while ($row = $db->fetchByAssoc($result)) {
                ...
                $image = "<img height=50 width=150 src='" . $site_url . "/" . $image_src . "'/>";
                $images_html .= $image;
                $imagevalues[] = $image;
            }
            return $images_html;
    ...
    public function getHelpDeskImgActionAllImages($id, $site_url, $action, $label) {
        $imagevalues = array();
        if (isset($id) and $id != "") {
            $db = PearDatabase::getInstance();
            $query = $this->getHelpDeskImgActionQuery($action);
            ...
            if($num_rows>0){
                if($action == "img_before_action") {
                    ...
                    $images_html = $label ."<br>";
                }
            }
            while ($row = $db->fetchByAssoc($result)) {
                $row = PDFMaker_Module_Model::fixStoredName($row);
                $image_src = $row["path"] . $row["attachmentsid"] . "_" . $row["storedname"];
                $image = "<br> <img height=200 width=300 src='" . $site_url . "/" . $image_src . "'/>";
                $images_html .= $image;
                $imagevalues[] = $image;
            }
            return $images_html;
    ...
    public function getHelpDeskImgActionQuery($action) {
        $query = "SELECT vtiger_attachments.*
                ...
                AND vtiger_attachments.subject = '".$action."' ";
        return $query;
    }
    ...
    public function getServiceReportsImage($id, $site_url) {
        ...
        if (isset($id) and $id != "") {
            ...
            while ($row = $db->fetchByAssoc($result)) {
                $row = PDFMaker_Module_Model::fixStoredName($row);
                $image_src = $row["path"] . $row["attachmentsid"] . "_" . $row["storedname"];
                $image = "<img height=400 width=450 src='" . $site_URL_NonHttp  . $image_src . "'/>";
                $imagevalues[] = $image;
            }
            return $imagevalues;
        } else {
    ...
    public function getSettingsForModule($module){
        ...
        $sql = "SELECT (margin_top * 10) AS margin_top,
              ...
              FROM vtiger_pdfmaker_settings INNER JOIN vtiger_pdfmaker 
              ON  vtiger_pdfmaker.templateid = vtiger_pdfmaker_settings.templateid 
              WHERE  vtiger_pdfmaker.module = ? AND vtiger_pdfmaker.module 
              IN ('Invoice','Quotes','SalesOrder','PurchaseOrder','ServiceReports','HelpDesk','RecommissioningReports') ";
