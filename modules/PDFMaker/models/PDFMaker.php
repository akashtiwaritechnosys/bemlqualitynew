...
class PDFMaker_PDFMaker_Model extends Vtiger_Module_Model {
    ...
    public function GetPreparedMPDF(&$mpdf, $record, $module, $language,$tempid) {
        ...
        //Change lineitem block in ServiceReports module 
        if($module == 'ServiceReports'){
            $body_html1 = htmlentities($pdf_content["body"]);
            if($record){
                global $adb;
                $getServiceReportLineItem = $adb->pquery("SELECT * FROM vtiger_inventoryproductrel WHERE id = ?", array($record));
                $serviceReportLineItemRows = $adb->num_rows($getServiceReportLineItem);
                
                $serviceReportLineItemBlock = '<table style="width: 763.333px;">
                    <thead>
                        <tr>
                            ...
                            <th style="width: 15%;">Fulfilment by</th>
                        </tr>
                    </thead>
                <tbody>';
                
                for ($i=0; $i < $serviceReportLineItemRows; $i++) { 
                    $sequence_no = $adb->query_result($getServiceReportLineItem, $i, 'sequence_no');
                    $productname = $adb->query_result($getServiceReportLineItem, $i, 'productname');
                    $comment = $adb->query_result($getServiceReportLineItem, $i, 'comment');
                    $quantity = $adb->query_result($getServiceReportLineItem, $i, 'quantity');
                    $sr_action_one = $adb->query_result($getServiceReportLineItem, $i, 'sr_action_one');
                    $sr_replace_action = $adb->query_result($getServiceReportLineItem, $i, 'sr_replace_action');
                    $serviceReportLineItemBlock .= '<tr>
                        <td style="width: 5%;">'.$sequence_no.'</td>
                        <td style="width: 25%;">'.$productname.'</td>
                        <td style="width: 25%;">'.$comment.'</td>
                        <td style="width: 10%;">'.str_replace('.000', '', $quantity).'</td>
                        <td style="width: 20%;">'.$sr_action_one.'</td>
                        <td style="width: 15%;">'.$sr_replace_action.'</td>
                    </tr>';
                }

                $serviceReportLineItemBlock .= '</tbody></table>';
            }
            $body_html1 = str_replace('$ServiceReportLineItemBlock$', $serviceReportLineItemBlock, $body_html1);
            $body_html = htmlspecialchars_decode($body_html1);
        }else{
