        ...
        if ($isentitytype) {
            ...
            if ($request->has('record') && !$request->isEmpty('record')) {
                ...
                if (!empty($module) && $module == $source_module) {
                    ...
                    $recInstance = Vtiger_Record_Model::getInstanceById($record);
                    if ($recInstance->get('is_enginereport') == '1') {
                        $viewer->assign('TEMPLATE_ISDEFAULT', 'PREDELIVERY');
                    } else {
                        $viewer->assign('TEMPLATE_ISDEFAULT', 'GENERAL INSPECTION');
                    }
