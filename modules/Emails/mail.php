...
function send_mail($module,$to_email,$from_name,$from_email,$subject,$contents,$cc='',$bcc='',$attachment='',$emailid='',$logo='', $useGivenFromEmailAddress=false,$useSignature = 'Yes',$inReplyToMessageId='')
{
	...
	// $mail->SMTPDebug = 2;
	$mail_status = MailSendPlainTele($module,$to_email,$from_name,$from_email,$subject,$contents,$cc,$bcc,$attachment,$emailid,$logo,$useGivenFromEmailAddress,$useSignature,$inReplyToMessageId);
	if ($mail_status != 1) {
...
function sendCommand($fp, $cmd, $expectedCode) {
	fwrite($fp, $cmd . "\r\n");
	$response = fgets($fp, 512);
	if (strpos($response, $expectedCode) !== 0) {
		// print_r("SMTP Error: $response");
	}
	return $response;
}

function MailSendPlainTele($module,$to_email,$from_name,$from_email,$subject,$contents,$cc,$bcc,$attachment,$emailid,$logo,$useGivenFromEmailAddress,$useSignature,$inReplyToMessageId) {
	$host = "10.80.2.16"; // SMTP server
	$port = 26; // or 587 for TLS, 465 for SSL
	$username = "otp-noreply@beml.co.in";
	$password = "Beml$12345";
	...
	fgets($fp, 512); // Server greeting
	sendCommand($fp, "HELO localhost", "250");
	// If authentication required
	sendCommand($fp, "AUTH LOGIN", "334");
	sendCommand($fp, base64_encode($username), "334");
	sendCommand($fp, base64_encode($password), "235");
	// Sender & recipient
	sendCommand($fp, "MAIL FROM:<$username>", "250");
	sendCommand($fp, "RCPT TO:<". $to_email .">", "250");
	// Email data
	sendCommand($fp, "DATA", "354");
	fwrite($fp, "Subject: " . $subject ."\r\n");
	fwrite($fp, "From: $username\r\n");
	fwrite($fp, "To: ". $to_email ."\r\n");
