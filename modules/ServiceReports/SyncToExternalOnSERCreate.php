...
function CreateOnlyNotification($entityData) {
    ...
    $sr_system_affected = $recordInfo['sr_system_affected'];
    if (strpos($sr_system_affected, 'ENGINE') !== false) {
        $query = "UPDATE vtiger_servicereports SET is_enginereport = ? WHERE servicereportsid=?";
    ...
    if ($notiType == '--') {
        ...
        if ($isSubmitted == '1' && $isRecomisionCreated == true) {
            ...
            $query = "UPDATE vtiger_troubletickets SET status = ?,ticket_recommision_date=? WHERE ticketid=?";
            $adb->pquery($query, array('Closed : Recommissioning Is Pending',date('Y-m-d'), $ticketId));
    ...
    if (!empty($exterAppNum)) {
        ...
        if ($isSubmitted == '1' && $isRecomisionCreated == false) {
            ...
            if ($responseObject['success'] == true) {
                ...
                $adb->pquery($query, array('Closed', date('Y-m-d'), $ticketId));
                changeStatus($ticketId, 'Closed');

                $timeinMinuts = getTimeDifferenceInMinutes($dataRow['createdtime'], date("Y-m-d H:i:s"));
                $adb->pquery('UPDATE vtiger_troubletickets SET time_insec_to_repair = ?
            ...
            $query = "UPDATE vtiger_troubletickets SET status = ?,ticket_recommision_date=? WHERE ticketid=?";
            $adb->pquery($query, array('Closed : Recommissioning Is Pending',date('Y-m-d'), $ticketId));
            changeStatus($ticketId, 'Closed : Recommissioning Is Pending');
    ...
    if ($recordInfo['sr_ticket_type'] == 'BREAKDOWN' || $recordInfo['sr_ticket_type'] == 'ERECTION AND COMMISSIONING' || $recordInfo['sr_ticket_type'] == 'PRE-DELIVERY') {
        ...
        if($recordInfo['sr_ticket_type'] == 'ERECTION AND COMMISSIONING' || $recordInfo['sr_ticket_type'] == 'PRE-DELIVERY'){
            $data['IT_CAUSE'] = json_encode(getAsArrayOfCodesErr());
        }
    }
    if (
        $notActualEquip == true && (
    ...
    if (($recordInfo['sr_ticket_type'] == 'SERVICE FOR SPARES PURCHASED'
            && $recordInfo['tck_det_purpose'] ==  'INSPECTION OF REJECTED SPARES')
        || $recordInfo['sr_ticket_type'] ==  'INSTALLATION OF SUB ASSEMBLY FITMENT'
    ) {
        $preDelPartNerArr = [];
        $accountId = $recordInfo['account_id'];
        $accountId = explode('x', $accountId);
    ...
    $log->debug("*****URL Is ***********" . json_encode($url) . "********");
    ...
    if (empty(trim($response['EX_QMNUM']))) {
        if (trim($response['IT_RETURN'][0]['MSGTYP']) == 'S' && trim($response['IT_RETURN'][1]['MESSAGE']) == "Issue in updating HMR, Please update manually") {
            $messageWithNotification = $response['IT_RETURN'][0]['MESSAGE'];
            $notificationNumber = (int)filter_var($messageWithNotification, FILTER_SANITIZE_NUMBER_INT);
            $isSubmitted = $recordInfo['is_submitted'];
            if ($recordInstance) {
            ...
            if ($responseObject['success'] == true) {
                $query = "UPDATE vtiger_troubletickets SET status = ? WHERE ticketid=?";
                $adb->pquery($query, array('In Progress', $ticketId));
                // changeStatus($ticketId, 'In Progress');
            }
            if ($isSubmitted == '1' && $isRecomisionCreated == false) {
...
function getAsArrayOfCodesErr() {
    global $adb;
    $products = [];
    $product = [];
    $product['LINE'] =  'CS01,0001';
    array_push($products, $product);
    return $products;
}
...
function createRecommisioningReport($id) {
    ...
    $focus->column_fields['servicereportsid'] = $salesorder_id;
    ...
    $adb->pquery($query, array('In Progress', $focus->id));
    return $focus->id;
}
