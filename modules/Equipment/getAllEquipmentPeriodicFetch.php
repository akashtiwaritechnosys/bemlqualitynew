...
function getAllEquipmentPeriodicFetch() {
	global $adb;
	...
	while ($fireRequest) {
		...
		foreach ($xml as $key => $value) {
			...
			$devision =  trim($value->{'STORT'});
			$devisionLabel = '';
			if ($devision == 'MY01') {
				$devisionLabel = 'TRUCK DIVISION';
			} else if ($devision == 'KE01') {
				$devisionLabel = 'EM DIVISION';
			}
			if ($num_rows > 0) {
				...
				$recordModel->set('eq_division', $devisionLabel);
				thischeckAndInsertOption($optionValue);

				if (!empty(trim($value->{'KUND1'}))) {
					...
					$recordModel->set('account_id', NULL);
				}
				...
				if (!empty(trim($value->{'KUND1'}))) {
					...
					$dataRow = $adb->fetchByAssoc($result, 0);
					if (empty($dataRow['accountid'])) {
						$recordModel->set('sold_to_party', NULL);
					} else {
						$recordModel->set('sold_to_party', $dataRow['accountid']);
					}
				} else {
					$recordModel->set('sold_to_party', NULL);
				}
				if (!empty(trim($value->{'KUND2'}))) {
					$sql = "select accountid from vtiger_account where external_app_num = ? ";
					$result = $adb->pquery($sql, array(trim($value->{'KUND2'})));
					$dataRow = $adb->fetchByAssoc($result, 0);
					if (empty($dataRow['accountid'])) {
						$recordModel->set('ship_to_party', NULL);
					} else {
						$recordModel->set('ship_to_party', $dataRow['accountid']);
					}
				} else {
					$recordModel->set('ship_to_party', NULL);
				}

				$sql = "select functionallocationsid from vtiger_functionallocations 
					INNER JOIN vtiger_crmentity on vtiger_crmentity.crmid = vtiger_functionallocations.functionallocationsid
					where functionallocation_name = ? and vtiger_crmentity.deleted = 0";
				...
				$sql = "select maintenanceplantid,plant_name 
					from vtiger_maintenanceplant 
					INNER JOIN vtiger_crmentity on vtiger_crmentity.crmid = vtiger_maintenanceplant.maintenanceplantid
					where plant_code = ? and vtiger_crmentity.deleted = 0";
				...
				$focus->column_fields['eq_division'] = $devisionLabel;
				$optionValue = trim($value->{'TYPBZ'});
				...
				if (!empty(trim($value->{'KUND1'}))) {
					...
					$focus->column_fields['account_id']  = NULL;
				}

				// sold party implementation
				if (!empty(trim($value->{'KUND1'}))) {
					$sql = "select accountid from vtiger_account where external_app_num = ? ";
					$result = $adb->pquery($sql, array(trim($value->{'KUND1'})));
					$dataRow = $adb->fetchByAssoc($result, 0);
					if (empty($dataRow['accountid'])) {
						$focus->column_fields['sold_to_party']  = NULL;
					} else {
						$focus->column_fields['sold_to_party']  = $dataRow['accountid'];
					}
				} else {
					$focus->column_fields['sold_to_party']  = NULL;
				}
				if (!empty(trim($value->{'KUND2'}))) {
					...
					$dataRow = $adb->fetchByAssoc($result, 0);
					if (empty($dataRow['accountid'])) {
						$focus->column_fields['ship_to_party']  = NULL;
					} else {
						$focus->column_fields['ship_to_party']  = $dataRow['accountid'];
					}
				} else {
					$focus->column_fields['ship_to_party']  = NULL;
				}
			...
			thiscreateWarrantyRecordIfNOtEits($value);
		}
		$itiration = $itiration + 1;
	}
}

function thiscreateWarrantyRecordIfNOtEits($value) {
	global $adb;
	$warantyCode = trim($value->{'MGANR'});
	$sql = 'select 1 from `vtiger_warrantydetails` 
	INNER JOIN vtiger_crmentity 
	ON vtiger_crmentity.crmid = vtiger_warrantydetails.warrantydetailsid
	where vtiger_crmentity.deleted = 0 and wr_warranty_code = ?';
...
function thischeckAndInsertOption($option) {
	global $adb;
