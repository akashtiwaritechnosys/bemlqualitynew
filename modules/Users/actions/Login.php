...
class Users_Login_Action extends Vtiger_Action_Controller {
	...
	function process(Vtiger_Request $request) {
		...
		if ($user->doLogin($password)) {
			...
			global $adb;
			$adb->pquery("UPDATE vtiger_users SET session_name = ? WHERE id = ?",
			[session_id(), $userid]);
			...
			header ('Location: index.php?module=Users&parent=Settings&view=SystemSetup'. '&fingerprint='.$_REQUEST['fingerprint']);
			exit();
		} else {
