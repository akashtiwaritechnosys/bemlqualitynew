...
class CustomerPortal_PreCustomerSignUp extends CustomerPortal_API_Abstract {
	...
	function process(CustomerPortal_API_Request $request) {
		...
		if (!empty($emailId)) {
			...
			$userTimezone = new DateTimeZone($current_user->time_zone ?? date_default_timezone_get());
			$serverTime = new DateTime('now', new DateTimeZone(date_default_timezone_get()));
			$serverTime->setTimezone($userTimezone);
			$userDateTime = $serverTime->format("d/m/Y h:i:s a");
			...
			$content = 'Dear User,<br><br> 
				...
				<br><br> 
				This request was made on ' . $userDateTime  . ' and will expire in next 15 Minute.<br><br> 
				Regards,<br> 
				BEML Support Team.<br>';
