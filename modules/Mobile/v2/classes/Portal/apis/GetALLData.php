...
class Portal_GetALLData_API extends Portal_Default_API {
	...
	public function process(Portal_Request $request) {
		...
		$hasEmptyData = false;
		$requestMeta = array_merge($request->getAll(),$_REQUEST);
		$result = Vtiger_Connector::getInstance()->ListModuleRecords($module, $request->get('label'), $request->get('q', array()), $params['fields'], $pageNo, $pageLimit,false,false,$requestMeta);
		
		$totalPages = ceil((float)$result['count'] / 10);
		$focusObj = CRMEntity::getInstance($module);
		$moduleIdColumn = $focusObj->table_index;
		for($i = 0; $i < $totalPages; $i++){
			$result = Vtiger_Connector::getInstance()->ListModuleRecords($module, $request->get('label'), $request->get('q', array()), $params['fields'], $i , $pageLimit,false,false,$requestMeta);
