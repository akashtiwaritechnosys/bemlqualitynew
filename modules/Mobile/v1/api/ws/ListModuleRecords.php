						...
						if (in_array($i, $referenceFields)) {
							...
							if ($record[$i] == '1') {
								if ($data['is_enginereport'] == '1') {
									...
									$record['report_url'] = $site_URL . "modules/Mobile/v1/DownloadPDFReport?module=PDFMaker&source_module=ServiceReports&action=IndexAjax&record=$recordID&mode=getPreviewContent&language=en_us&generate_type=attachment&igtempid=7&access_token=" . $request->get('access_token') . "&useruniqueid=" . $request->get('useruniqueid');
								} else {
									$record['report_status'] = 'Submitted';
									$record['report_url'] = $site_URL . "modules/Mobile/v1/DownloadPDFReport?module=PDFMaker&source_module=ServiceReports&action=IndexAjax&record=$recordID&mode=getPreviewContent&language=en_us&generate_type=attachment&igtempid=12&access_token=" . $request->get('access_token') . "&useruniqueid=" . $request->get('useruniqueid');
								}
							} else {
								if ($data['is_enginereport'] == '1') {
							...
							$record['doc_url'] = $site_URL . 'modules/Mobile/v1/DownloadFile?record='.$data['notesid'].'&access_token='.$request->get('access_token').'&useruniqueid='.$request->get('useruniqueid');
