...
class Mobile_WS_UserSignUp extends Mobile_WS_Controller {
    ...
    function process(Mobile_API_Request $request) {
        ...
        if ($status == true) {
            ...
            if ($otp == $otpFromDataBase) {
                ...
                $now = strtotime("Now");
                // $badgeNoAndMobile = IGisBadgeExits($handlerData['badge_no'],$handlerData['phone']);
                // if (!empty($badgeNoAndMobile)) {
                //     if (isset($handlerData['badge_no']) && !empty($handlerData['badge_no']) && $badgeNo == $handlerData['badge_no']) {
                //         $response->setError(100, "Badge Number Already Exits");
                //         return $response;
                //     } else if (isset($$handlerData['phone']) && !empty($handlerData['phone']) && $mobileNo == $handlerData['phone']) {
    ...
    function getParentRoleUserId($roleName, $data) {
        ...
        if (empty($userId) || $userId == 1) {
            $userid = $this->checkForOtherUserWhoHasAuthority($parentRole);
            return $userid;
