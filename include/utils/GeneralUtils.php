...
function getSAPBasedOnType($type, $purposeValue) {
	...
	switch ($type) {
		case 'GENERAL INSPECTION':
			$SAPDefalutValue = 'Z3 -- GENERAL INSPECTION';
			break;
		case 'PRE-DELIVERY':
			$SAPDefalutValue = 'ZB -- PRE-DELIVERY';
			break;
		case 'ERECTION AND COMMISSIONING':
			$SAPDefalutValue = 'ZE -- ERECTION AND COMMISSIONING';
			break;
		case 'DESIGN MODIFICATION':
			$SAPDefalutValue = 'Z2 -- DESIGN MODIFICATION';
			break;
		case 'PERIODICAL MAINTENANCE':
			$SAPDefalutValue = 'Z4 -- PERIODICAL MAINTENANCE';
			break;
		default:
			$SAPDefalutValue = '';
			break;
...
function changeNotifiationStatus($status, $notificationNumber, $id) {
	...
	$data = array(
		...
		'IM_NOTIFICATION' => '000'. $notificationNumber
	);
	...
	curl_close($resource);
	$log->debug("*****Response Recived From SAP***********". var_dump($responseUnEncoded) ."********");
	$responseObject = [];
	$responseObject['success'] = false;
...
function IGisBadgeExitsBeforeCreate($badgeNo, $phone) {
	global $adb;
	$sql = 'select badge_no,phone,approval_status from vtiger_serviceengineer 
	inner join vtiger_crmentity on vtiger_crmentity.crmid = vtiger_serviceengineer.serviceengineerid
	where (badge_no = ? or phone = ?) and vtiger_crmentity.deleted = 0';
