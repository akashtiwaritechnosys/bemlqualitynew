...
function changeStatus($entityId, $newStatus) {
	global $adb;
	$sql = "SELECT * FROM status_history WHERE entity_id = ? ORDER BY changed_at DESC LIMIT 1";
	$rs = $adb->pquery($sql, [$entityId]);
	$lastStatus = $rs ? $rs->fields : false;

	$now = new DateTime();
	if ($lastStatus && !empty($lastStatus['changed_at'])) {
		$lastChanged = new DateTime($lastStatus['changed_at']);
		$duration = $now->getTimestamp() - $lastChanged->getTimestamp();
		// $updateSql = "UPDATE status_history SET duration_seconds = ? WHERE id = ?";
		// $adb->pquery($updateSql, [$duration, $lastStatus['id']]);
		$oldStatus = $lastStatus['new_status'];
	} else {
		$oldStatus = null;
		$duration = 0; // No previous status, so no duration
	}

	$insertSql = "INSERT INTO status_history (entity_id, old_status, new_status, changed_at, 
...
function saveInventoryProductDetails(&$focus, $module, $update_prod_stock='false', $updateDemand='')
{
	...
	for($i=1; $i<=$tot_no_prod; $i++)
	{
		...
		if (!$prod_id) {
			$prod_name = vtlib_purify($_REQUEST['manual_part_no'.$i]);
			if (empty($prod_name)) {
				$prod_name = vtlib_purify($_REQUEST['manual_part_no_so' . $i]);
			}
		}
...
function saveInventoryRSOProductDetails(&$focus, $module, $update_prod_stock = 'false', $updateDemand = '') {
	global $log, $adb;
	$id = $focus->id;
...
function saveInventoryProductDetailsFromMobile($LineItemsOther, $module, $createdId, $recordValues) {
	...
	$formate = $current_user->date_format;
	global $adb;
	// $adb->pquery("delete from vtiger_inventoryproductrel_other where id=?", array($createdId));
	if ($module == 'FailedParts' || $module == 'ServiceReports' || $module == 'RecommissioningReports') {
		$tabId = getTabId($module);
		$sql = "SELECT * FROM `vtiger_field` LEFT JOIN vtiger_blocks
         on vtiger_blocks.blockid = vtiger_field.block where vtiger_field.tabid = ? 
