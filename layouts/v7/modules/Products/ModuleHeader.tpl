								...
								<ul class="nav navbar-nav">
									...
									{if $USER_MODEL->isAdminUser()}
										{if $smarty.request.view eq 'List'}
											...
											<li>
												<button id="{$MODULE}_basicAction_{Vtiger_Util_Helper::replaceSpaceWithUnderScores('Sync Alt Materials')}" type="button" class="btn addButton btn-soft-info module-btn"
														{if stripos($BASIC_ACTION->getUrl(), 'javascript:')===0}
													onclick='{$BASIC_ACTION->getUrl()|substr:strlen("javascript:")};'
														{else}
													onclick="Products_List_Js.syncAltMaterials(1)"
														{/if}>
													<div class="fa {$BASIC_ACTION->getIcon()}" aria-hidden="true"></div>&nbsp;&nbsp;
													Sync ALT Materials
												</button>
