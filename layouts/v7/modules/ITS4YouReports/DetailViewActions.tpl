            ...
            <div class="row">
                        ...
                        <div class="btn-group">
                            ...
                            {foreach item=DETAILVIEW_LINK from=$DETAILVIEW_ACTIONS}
                                ...
                                <button {if $LINK_URL} onclick='window.location.href = "{$LINK_URL}"' {/if} type="button"
                                               ...
                                               {if $LINK_ICON_CLASS eq 'vtGlyph vticon-attach' && ITS4YouReports_Functions_Helper::php7_count($DASHBOARD_TABS) gt 1} dropdown-toggle{/if}"
                                        ...
                                        {else}{$DETAILVIEW_LINK->get('linktitle')}{/if}" {if $LINK_ICON_CLASS eq 'vtGlyph vticon-attach' && ITS4YouReports_Functions_Helper::php7_count($DASHBOARD_TABS) gt 1 }data-toggle="dropdown"{/if}
                                    {if $LINK_ICON_CLASS eq 'vtGlyph vticon-attach'}data-dashboard-tab-count='{ITS4YouReports_Functions_Helper::php7_count($DASHBOARD_TABS)}'{/if}
                                    style="
                                        {if 0 < $BTN_I}
                                            margin-left:5px;
                                        {/if}
                ...
                <div class='col-lg-3'>
                    <span class="pull-right">
                        <div class="btn-toolbar">
                            <div class="btn-group">
                                {if 'tabular' eq $REPORT_MODEL->getReportType()}
                                    {foreach item=DETAILVIEW_LINK from=$DETAILVIEW_LINKS}
                                        {assign var=LINKNAME value=$DETAILVIEW_LINK->getLabel()}
                                        <span class="btn-group">
                                        {if 'custom_report' eq $REPORT_MODEL->getReportType()}
                                            <button class="btn btn-default reportActions" data-href="{$DETAILVIEW_LINK->getUrl()}&source={$REPORT_MODEL->getReportType()}"
                                                    style="margin-left:5px;" type="button" data-toggle="dropdown" name="{$DETAILVIEW_LINK->get('linkname')}" >
                                                        {$LINKNAME}
                                                    </button>
                                        {else}
                                            <ul class="nav navbar-nav navbar-right">
                                                <li>
                                                    <button class="btn btn-default dropdown-toggle"
                                                            style="margin-left:5px;" type="button" data-toggle="dropdown" >
                                                        {$LINKNAME}
                                                        &nbsp;<span class="caret"></span>
                                                    </button>
                                                    <ul class="dropdown-menu">
                                                        <li>
                                                            <a href="javascript:void(0);" data-href="{$DETAILVIEW_LINK->getUrl()}&data=base" data-type="base" name="{$DETAILVIEW_LINK->get('linkname')}" class="reportActions" >
                                                                <i class='{$ICON_CLASS}' style="font-size:13px;"></i>&nbsp; {vtranslate('LBL_EXPORT_REPORTED_RECORDS', 'ITS4YouReports')}
                                                            </a>
                                                        </li>
                                                        <li>
                                                            <a href="javascript:void(0);" data-href="{$DETAILVIEW_LINK->getUrl()}&data=all" data-type="all" name="{$DETAILVIEW_LINK->get('linkname')}" class="reportActions" >
                                                                <i class='{$ICON_CLASS}' style="font-size:13px;"></i>&nbsp; {vtranslate('LBL_ALL',$MODULENAME)}
                                                            </a>
                                                        </li>
                                                        <li>
                                                            <a href="javascript:void(0);" data-href="{$DETAILVIEW_LINK->getUrl()}" data-type="all" data-background-export="true" name="{$DETAILVIEW_LINK->get('linkname')}" class="reportActions" >
                                                                <i class='{$ICON_CLASS}' style="font-size:13px;"></i>&nbsp; {vtranslate('LBL_BACKGROUND_EXPORT', 'ITS4YouReports')}
                                                            </a>
                                                        </li>
                                                    </ul>
                                                </li>
                                    ...
                                    {/foreach}
                                {elseif 'custom_report' neq $REPORT_MODEL->getReportType()}
                                    {foreach item=DETAILVIEW_LINK from=$DETAILVIEW_LINKS}
                                        {assign var=LINKNAME value=$DETAILVIEW_LINK->getLabel()}
                                            {if 'generateMap' neq $DETAILVIEW_LINK->get('id')}
                                                <ul class="nav navbar-nav navbar-right">
                                                    <li>
                                                        <button class="btn btn-default dropdown-toggle"
                                                                style="margin-left:5px;" type="button" data-toggle="dropdown" >
                                            ...
                                            {else}
                                                {assign var=LINKNAME value=$DETAILVIEW_LINK->getLabel()}
                                                <button class="btn btn-default reportActions" name="{$DETAILVIEW_LINK->get('linkname')}"
