{strip}
	...
	<td>
		{if $COMMENT_EDITABLE}
			<div><textarea id="{$comment}" name="{$comment}" readonly style="pointer-events: none;background-color:#eeeeee" class="lineItemCommentBox">{decode_html($data.$comment)}</textarea></div>
		{/if}
	</td>
	<td>
	...
	{foreach key=LINEITEM_CUSTOM_FIELDKEY item=LINEITEM_CUSTOM_FIELD from=$LINEITEM_CUSTOM_FIELDS}
		<td>
			...
			{elseif $LINEITEM_CUSTOM_FIELD['uitype'] eq '16'}
				<select style="min-width: 250px;" id="{${$fieldName}}" class="select2-container inputElement select2 inputElement picklistfield" name="{${$fieldName}}" class="inputElement picklistfield" data-fieldtype="picklist">
