...
{strip}
   <div class="fc-overlay-modal modal-content">
      <div class="modal-content">
         {assign var=HEADER_TITLE value={vtranslate('LBL_ADD_TASKS_FOR_WORKFLOW', $QUALIFIED_MODULE)}|cat:" ->
      "|cat:{vtranslate($TASK_TYPE_MODEL->get('label'),$QUALIFIED_MODULE)}}
         {include file="ModalHeader.tpl"|vtemplate_path:$MODULE TITLE=$HEADER_TITLE}
         <div class="modal-body editTaskBody" style="max-height:400px;overflow:scroll">
                        ...
                        <div class="col-sm-9 col-xs-9">
                           <div class="row">
                              <div class="col-lg-2 col-sm-4">
                                 <label>{vtranslate('LBL_TASK_TITLE',$QUALIFIED_MODULE)}</label><span
                                    class="redColor">*</span></div>
                              <div class="col-lg-6 col-sm-8"><input name="summary" class="inputElement"
                                    data-rule-required="true" type="text" value="{$TASK_MODEL->get('summary')}" /></div>
                           </div>
                        </div>
                     </div>
                     {if $TASK_TYPE_MODEL->get('tasktypename') eq "VTEmailTask" && $TASK_OBJECT->trigger != null}
                        ...
                        <div class="row form-group">
                           <div class="col-sm-9 col-xs-9">
                              <div class="row">
                                 <div class="col-lg-2 col-sm-4">
                                    <label>{vtranslate('LBL_DELAY_ACTION', $QUALIFIED_MODULE)}</label> </div>
                                 <div class="col-sm-10 col-xs-10">
                                    <div class="row">
                                       ...
                                       <div class="col-sm-10 col-xs-10 {if $trigger neq null}show {else} hide {/if}"
                                          id="checkSelectDateContainer">
                                          <div class="row">
                                             <div class="col-lg-2 col-sm-4">
                                                <div class="row">
                                                   <div class="col-lg-6 col-sm-8" style="padding: 0px;">
                                                      <input class="inputElement" type="text" name="select_date_days"
                                                         value="{$days}" data-rule-WholeNumber=="true">&nbsp;
                                                   </div>
                                                   <div class="alignMiddle col-sm-5 col-xs-5"
                                                      style="padding: 0px; margin-left: 2px;"><span
                                                         style="position:relative;top:3px;">&nbsp;{vtranslate('LBL_DAYS',$QUALIFIED_MODULE)}</span>
                                                   </div>
                                                </div>
                                             </div>
                                             <div class="col-lg-3 col-sm-3">
                                                <select class="select2" name="select_date_direction" style="width: 100px">
                                                   <option {if $direction eq 'after'} selected="" {/if} value="after">
                                                      {vtranslate('LBL_AFTER',$QUALIFIED_MODULE)}</option>
                                                   <option {if $direction eq 'before'} selected="" {/if} value="before">
                                                      {vtranslate('LBL_BEFORE',$QUALIFIED_MODULE)}</option>
                                                </select>
                                             </div>
                                             <div class="col-lg-6 col-sm-8 marginLeftZero">
                                                <select class="select2" name="select_date_field">
                                                   {foreach from=$DATETIME_FIELDS item=DATETIME_FIELD}
                                                      <option {if $trigger['field'] eq $DATETIME_FIELD->get('name')} selected=""
                                                         {/if} value="{$DATETIME_FIELD->get('name')}">
                                                         {vtranslate($DATETIME_FIELD->get('label'), $DATETIME_FIELD->getModuleName())}
                                                      </option>
                                                   {/foreach}
                                                </select>
                                             </div>
