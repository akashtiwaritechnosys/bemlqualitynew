...
{strip}
	...
	{foreach key=BLOCK_LABEL_KEY item=FIELD_MODEL_LIST from=$RECORD_STRUCTURE}
		...
		{if $BLOCK eq null or $FIELD_MODEL_LIST|php7_count lte 0 or $BLOCK_LABEL_KEY eq 'LBL_PRICING_INFORMATION' or $BLOCK_LABEL_KEY eq 'Aggregate Details'}
		...
		{/if}
		{if $BLOCK_LABEL_KEY eq 'Aggregate Details' and $EquipmentCategory eq 'S'}
			<div>
				<h4 class="textOverflowEllipsis maxWidth50">
					<img class="cursorPointer alignMiddle blockToggle {if !($IS_HIDDEN)} hide {/if}" src="{vimage_path('arrowRight.png')}" data-mode="hide" data-id={$BLOCK_LIST[$BLOCK_LABEL_KEY]->get('id')}>
					<img class="cursorPointer alignMiddle blockToggle {if ($IS_HIDDEN)} hide {/if}" src="{vimage_path('arrowdown.png')}" data-mode="show" data-id={$BLOCK_LIST[$BLOCK_LABEL_KEY]->get('id')}>&nbsp;
			...
			<hr>
			{include file="LineItemsDetailShortDam2.tpl"|@vtemplate_path:'Equipment'}
		{/if}
		<br>
	{/foreach}
{/strip}