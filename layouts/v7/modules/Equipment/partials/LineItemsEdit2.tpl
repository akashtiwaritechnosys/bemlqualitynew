...
{foreach item=LINEITEM_CUSTOM_FIELD from=$LINEITEM_CUSTOM_FIELDS_OTHER1}
	...
	{/if}
{/foreach}
{assign var=LINEITEM_FIELDS value=$RECORD_STRUCTURE['LBL_ITEM_DETAILS']|default:[]}
{if $LINEITEM_FIELDS && $LINEITEM_FIELDS['image']}
...
{/if}
{if $LINEITEM_FIELDS && $LINEITEM_FIELDS['productid']}
...
{/if}
{if $LINEITEM_FIELDS && $LINEITEM_FIELDS['quantity']}
...
{if $REPORTTYPE eq 'WARRANTY CLAIM FOR SUB ASSEMBLY / OTHER SPARE PARTS'}
	...
	{assign var=COL_SPAN1 value=($COL_SPAN1)-1}
{/if}
{if $LINEITEM_FIELDS && $LINEITEM_FIELDS['purchase_cost']}
...
{/if}
{if $LINEITEM_FIELDS && $LINEITEM_FIELDS['listprice']}
...
{/if}
{if $LINEITEM_FIELDS && $LINEITEM_FIELDS['margin']}
...
<input type="hidden" disabled="disabled" id="fildNamesOfCustPickFieldsInfoOther" value='{Vtiger_Util_Helper::toSafeHTML(Vtiger_Functions::jsonEncode($LINEITEM_CUSTOM_OTHER_PICK_FIELDS))}'>
...
<div name='editContent'>
	...
	{assign var=BLOCK_FIELDS value=$RECORD_STRUCTURE.$LINE_ITEM_BLOCK_LABEL|default:[]}
	{assign var=ITEMBLOCKNAMEANOTHER value='Contracts Avalibilty Values'}
	{if ($BLOCK_FIELDS|php7_count gt 0) or 1 == 1}
			...
			<div class="lineitemTableContainerValuesHolder">
				<table class="table table-bordered hide" id="lineItemTab2">
					...
					<tbody class="appendRows">
						...
						{if $RELATED_PRODUCTS_OTHER1 && (php7_count($RELATED_PRODUCTS_OTHER1) gt 0)}
							{foreach key=row_no item=data from=$RELATED_PRODUCTS_OTHER1}
								<tr id="row{$row_no}" data-row-num="{$row_no}" class="lineItemRow" {if $data && $data["entityType$row_no"] eq 'Products'}data-quantity-in-stock={$data["qtyInStock$row_no"]}{/if}>
