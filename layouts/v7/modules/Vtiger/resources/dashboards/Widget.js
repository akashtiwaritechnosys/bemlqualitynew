...
Vtiger.Class(
  "Vtiger_Widget_Js",
  {
    ...
    getInstance: function (container, widgetName, moduleName) {
      ...
      var moduleClass =
        window[moduleName + "_" + widgetClassName + "_Widget_Js"];
  ...
  },
  {
    ...
    restrictContentDrag: function () {
      this.getContainer().on("mousedown.draggable", function (e) {
        ...
        var isHeaderElement =
          element.closest(".dashboardWidgetHeader").length > 0 ? true : false;
    ...
    refreshWidget: function () {
      ...
      if (widgetFilters.length > 0) {
        ...
        widgetFilters.each(function (index, domElement) {
          ...
          if (
            (widgetFilter.is(":radio") || widgetFilter.is(":checkbox")) &&
            !widgetFilter.is(":checked")
          ) {
            return true;
          }
      ...
      app.request.post(params).then(function (err, data) {
         ...
         */
        var widgetChartContainer = contentContainer.find(
    ...
    registerFilter: function () {
      ...
      dateRangeElement.attr(
        "data-date-format",
        thisInstance.getUserDateFormat()
      );
    },
    ...
    registerWidgetPostLoadEvent: function (container) {
      var thisInstance = this;
      container
        .off(Vtiger_Widget_Js.widgetPostLoadEvent)
        .on(Vtiger_Widget_Js.widgetPostLoadEvent, function (e) {
...
Vtiger_Widget_Js(
  "Vtiger_TopPotentials_Widget_Js",
  {},
  {
    postLoadWidget: function () {
      this._super();
    ...
    registerLoadMore: function () {
      ...
      loadMoreHandler.click(function () {
        ...
        if (!jQuery.isEmptyObject(filterData)) {
          ...
          params.data = jQuery.extend(
            params.data,
            thisInstance.getFilterData()
          );
        }
    ...
    loadChart: function () {
      ...
      var options = {
        ...
        dataLabels: {
          enabled: true,
          style: {
            colors: ["#000000"], // 👈 this sets label color
            fontSize: "16px",
            fontWeight: "600",
          },
        },
      ...
      var chart = new ApexCharts(
        document.querySelector("#OpenTickets"),
        options
      );
      chart.render();
    },
...
Vtiger_Barchat_Apex_Widget_Js(
  "Vtiger_LeadsByIndustry_Widget_Js",
  {},
  {
    loadChart: function () {
      var data = this.generateChartData();
      ...
      var options = {
        chart: {
          ...
          height: "100%",
          width: "100%",
          offsetY: -50, // Move chart up
        },
        legend: {
          position: "top",
          offsetY: -22, // Move legend down
        },
        ...
        plotOptions: {
          bar: {
            distributed: true, // IMPORTANT: enables per-bar coloring
            borderRadius: 4,
            horizontal: false,
            horizontal: true,
          },
      ...
      var chart = new ApexCharts(
        document.querySelector("#LeadsByIndustry"),
        options
      );
      chart.render();
    },
...
Vtiger_Barchat_Apex_Widget_Js(
  "Vtiger_LeadsBySource_Widget_Js",
  {},
  {
    loadChart: function () {
      var data = this.generateChartData();
      ...
      var chart = new ApexCharts(
        document.querySelector("#LeadsBySource"),
        options
      );
      chart.render();
    },
...
Vtiger_Barchat_Apex_Widget_Js(
  "Vtiger_TicketsByStatus_Widget_Js",
  {},
  {
    loadChart: function () {
      var data = this.generateChartData();
...
Vtiger_Barchat_Widget_Js(
  "Vtiger_LeadsByStatus_Widget_Js",
  {},
  {
    loadChart: function () {
      var data = this.generateChartData();
      ...
      var chart = new ApexCharts(
        document.querySelector("#LeadsByStatus"),
        options
      );
      chart.render();
    },
...
Vtiger_Widget_Js("Vtiger_MultiBarchat_Widget_Js", {
    ...
    for (j in stages) {
      ...
      for (i in users) {
        ...
        for (var k in data) {
          var userData = data[k];
          if (
            userData.sales_stage == stages[j] &&
            userData.last_name == users[i]
          ) {
  ...
  loadChart: function () {
    ...
    this.getPlotContainer(false).data(
      "widget-data",
      JSON.stringify(this.getCharRelatedData())
    );
...
Vtiger_Barchat_Apex_Widget_Js(
  ...
  {},
  {
    loadChart: function () {
      ...
      var chart = new ApexCharts(
        document.querySelector("#FunnelAmount"),
        options
      );
      chart.render();
    },
...
Vtiger_MultiBarchat_Widget_Js(
  "Vtiger_GroupedBySalesPerson_Widget_Js",
  {},
  {
    getCharRelatedData: function () {
      var container = this.getContainer();
      ...
      // Collect unique users and stages
      for (var i = 0; i < data.length; i++) {
        if ($.inArray(data[i].last_name, users) == -1) {
          users.push(data[i].last_name);
        }
      ...
      for (var i in stages) {
        ...
        for (var j in users) {
          ...
          for (var k in data) {
            var userData = data[k];
            if (
              userData.sales_stage == stages[i] &&
              userData.last_name == users[j]
            ) {
    ...
    loadChart: function () {
      ...
      const options = {
        chart: {
          ...
          events: {
            dataPointSelection: function (event, chartContext, config) {
              let link = chartRelatedData.links[config.seriesIndex][config.dataPointIndex];
              if (link) {
        ...
        colors: ["var(--primary-color)", "#00E396", "#FEB019", "#FF4560"],
        tooltip: {
          shared: true,
          intersect: false,
        },
      ...
      const chart = new ApexCharts(
        document.querySelector("#GroupedBySalesPerson"),
        options
      );
      chart.render();
    },
...
Vtiger_MultiBarchat_Widget_Js(
  ...
  {},
  {
    ...
    loadChart: function () {
      ...
      const options = {
        chart: {
          ...
          events: {
            dataPointSelection: function (event, chartContext, config) {
              window.open(
                chartRelatedData["links"][config.dataPointIndex][
                  config.seriesIndex
                ],
                "_blank"
              );
      ...
      const chart = new ApexCharts(
        document.querySelector("#PipelinedAmountPerSalesPerson"),
        options
      );
      chart.render();
    },
...
Vtiger_Widget_Js(
  ...
  {},
  {
    generateChartData: function () {
      ...
      for (var index in data) {
        var row = data[index];
        row[0] = parseFloat(row[1]);
        xLabels.push(app.getDecodedValue(row[2]));
        chartData.push(row[0]);
        if (parseInt(row[0]) > yMaxValue) {
          yMaxValue = parseInt(row[0]);
        }
...
Vtiger_Funnel_Apex_Widget_Js(
  "Vtiger_GroupedBySalesStage_Widget_Js",
  {},
  {
    loadChart: function () {
      var data = this.generateChartData();
      ...
      const chart = new ApexCharts(
        document.querySelector("#GroupedBySalesStage"),
        options
      );
      chart.render();
    },
...
Vtiger_Widget_Js(
  ...
  {},
  {
    ...
    postResizeWidget: function () {
      ...
      slimScrollDiv.css({ height: adjustedHeight });
      widgetContent
        .find(".dashboard_notebookWidget_viewarea")
    ...
    reinitNotebookView: function () {
      var self = this;
      jQuery(".dashboard_notebookWidget_edit", this.container).click(
        function () {
          self.editNotebookContent();
        }
      );
      jQuery(".dashboard_notebookWidget_save", this.container).click(
        function () {
...
Vtiger_History_Widget_Js(
  "Vtiger_OverdueActivities_Widget_Js",
  {},
  {
    registerLoadMore: function () {
      var thisInstance = this;
      ...
      parent.on("click", 'a[name="history_more"]', function (e) {
        ...
        app.request.post({ url: params }).then(function (err, data) {
          ...
          var dashboardWidgetData = parent.find(
            ".dashboardWidgetContent .dashboardWidgetData"
          );
          var scrollTop =
            dashboardWidgetData.height() * dashboardWidgetData.length - 100;
          widgetContent.mCustomScrollbar("destroy");
...
Vtiger_OverdueActivities_Widget_Js(
  "Vtiger_CalendarActivities_Widget_Js",
  {},
  {}
);
