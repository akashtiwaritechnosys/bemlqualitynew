...
Vtiger_Index_Js("Vtiger_Edit_Js", {
    ...
    registerValidation: function () {
        ...
        this.formValidatorInstance = editViewForm.vtValidate({
            submitHandler: function () {
                ...
                editViewForm.find('.saveAndNewButton').attr('disabled', true);
                return true;
            }
        });

        editViewForm.on('click', '.saveAndNewButton', function(e) {
             // Create hidden input for save and new
             if(editViewForm.find('[name="saveAndNew"]').length == 0) {
                 jQuery('<input>').attr({
                     type: 'hidden',
                     name: 'saveAndNew',
                     value: 'true'
                 }).appendTo(editViewForm);
             }
             editViewForm.submit();
        });
    },
    ...
    */
    ...
    registerFileElementChangeEvent: function (container) {
        ...
        container.on('change', 'input[name="imagename1[]"],input[name="sentdocument"]', function (e) {
            if (e.target.type == "text") return false;
            var moduleName = jQuery('[name="module"]').val();
            if (moduleName == "Products") return false;
            Vtiger_Edit_Js.file = e.target.files[0];
            var element = container.find('[name="imagename1[]"],input[name="sentdocument"]');
            //ignore all other types than file 
            if (element.attr('type') != 'file') {
                return;
            }
     ...
     registerNameValidation: function () {
      const nameRegex = /^[A-Za-z ]+$/;

      $('input[name="firstname"], input[name="lastname"]').on(
        "input",
        function () {
          let value = $(this).val();
       ...
       registerPhoneMobileValidation: function () {

        var fields = ['phone', 'mobile', 'cf_1185', 'homephone', 'otherphone', 'assistantphone', 'fax'];
      ...
      registerforfieldmaxlanght: function () {
          const MAX_LENGTH = 50;

          jQuery(document).on('input', 'input[type="text"], input[type="email"]', function () {
              ...
              if (value.length > MAX_LENGTH) {
                  ...
                  app.helper.showErrorNotification({
                      message: 'Maximum ' + MAX_LENGTH + ' characters allowed'
                  });
              }
          });
      ...
      registerWebsiteValidation: function () {
        jQuery('[name="website"]').on('blur', function () {
            var website = jQuery(this).val().trim();
            if (website === '') return;
            ...
            if (!urlPattern.test(website)) {
                app.helper.showErrorNotification({
                    message: "Please enter a valid website URL (example: https://www.example.com)"
                });
    ...
    registerProjectTaskNumberValidation: function () {
        let form = this.getForm();
        let field = form.find('[name="projecttasknumber"]');
     ...
     registerPositiveFloatValidation: function () {

        let fields = ['cf_1215', 'cf_1217'];

        fields.forEach(function (fieldName) {

            let field = jQuery('[name="' + fieldName + '"]');
    ...
    registerEvents: function (callParent) {
        ...
        this.registerNameValidation();
        this.registerPhoneMobileValidation();
        this.registerforfieldmaxlanght();
        this.registerWebsiteValidation();
        this.registerProjectTaskNumberValidation();
        this.registerPositiveFloatValidation();
    }
});
