{strip}
	...
	<td style="text-align:center;">
		...
		<input type="hidden" id="cust_role" class="cust_role" value="{$cust_role}" />
		<input type="hidden" id="office" class="office" value="{$office}" />
				...
				<div class="row">
					<div class="col-lg-10">
						<div class="input-group" style="width:100%">
							...
							{/if}
							<input type="hidden" id="{$igLineItemId}" name="{$igLineItemId}" value="{$data.$igLineItemId}"/>
	...
	{foreach key=LINEITEM_CUSTOM_FIELDKEY item=LINEITEM_CUSTOM_FIELD from=$LINEITEM_CUSTOM_FIELDS}
		<td a="{$LINEITEM_CUSTOM_FIELD['fieldlabel']}">
			...
			{elseif $LINEITEM_CUSTOM_FIELD['uitype'] eq '16'}
			 	{if $fieldName neq 'action_taken_by_sm' AND $LINEITEM_CUSTOM_FIELD['fieldlabel'] neq 'Present Status' AND $LINEITEM_CUSTOM_FIELD['fieldlabel'] neq 'Action Taken by DSM'}
					<div id="{$fieldName}DivCla" a="{$LINEITEM_CUSTOM_FIELD['fieldlabel']}" {if $LINEITEM_CUSTOM_FIELD['hideInitialDisplay'] eq 'true'} class="hide" {/if}>
						<select {if $LINEITEM_CUSTOM_FIELD['editable'] eq 0} readonly style="pointer-events: none;background-color:#eeeeee;min-width: 100px;" {/if}
				...
				{elseif $fieldName eq 'action_taken_by_sm'}
					{assign var="dependentDopDownValuesOfActiontaken" value="{Vtiger_Functions::getDropDownValuesOfDependentField($data.{'validated_part_no'|cat:$row_no})}"}
					{if $dependentDopDownValuesOfActiontaken eq 'FRR' }	
						<div id="{$fieldName}DivCla" a="{$LINEITEM_CUSTOM_FIELD['fieldlabel']}" {if $LINEITEM_CUSTOM_FIELD['hideInitialDisplay'] eq 'true'} class="hide" {/if}>
							<select {if $LINEITEM_CUSTOM_FIELD['editable'] eq 0} readonly style="pointer-events: none;background-color:#eeeeee;min-width: 100px;" {/if}
							data-rule-required=true style="min-width: 150px;" id="{${$fieldName}}" 
							class="select2-container {if $row_no neq 0}select2{/if} inputElement picklistfield {$fieldName}" 
							name="{${$fieldName}}" data-extraname="{$fieldName}" data-fieldtype="picklist">
								<option value="">{vtranslate('LBL_SELECT_OPTION','Vtiger')}</option>
								{foreach  key=PICKLIST_FIELDKEY item=PICKLIST_FIELD_ITEM from=$dependentDopDownValuesOfActionFRR}
									<option {if trim(decode_html($data.${$fieldName})) eq $PICKLIST_FIELDKEY} selected {/if} value="{$PICKLIST_FIELDKEY}">{$PICKLIST_FIELDKEY}</option>
								{/foreach}
							</select>
						</div>
					{elseif $dependentDopDownValuesOfActiontaken eq 'FRN'}
						<div id="{$fieldName}DivCla" a="{$LINEITEM_CUSTOM_FIELD['fieldlabel']}" {if $LINEITEM_CUSTOM_FIELD['hideInitialDisplay'] eq 'true'} class="hide" {/if}>
							<select {if $LINEITEM_CUSTOM_FIELD['editable'] eq 0} readonly style="pointer-events: none;background-color:#eeeeee;min-width: 100px;" {/if}
							data-rule-required=true style="min-width: 150px;" id="{${$fieldName}}" 
							class="select2-container {if $row_no neq 0}select2{/if} inputElement picklistfield {$fieldName}" 
							name="{${$fieldName}}" data-extraname="{$fieldName}" data-fieldtype="picklist">
								<option value="">{vtranslate('LBL_SELECT_OPTION','Vtiger')}</option>
								{foreach  key=PICKLIST_FIELDKEY item=PICKLIST_FIELD_ITEM from=$dependentDopDownValuesOfActionFRN}
									<option {if trim(decode_html($data.${$fieldName})) eq $PICKLIST_FIELDKEY} selected {/if} value="{$PICKLIST_FIELDKEY}">{$PICKLIST_FIELDKEY}</option>
								{/foreach}
							</select>
						</div>
					{elseif $typeOfLineItem eq 'FRR'}
						<div id="{$fieldName}DivCla" a="{$LINEITEM_CUSTOM_FIELD['fieldlabel']}" {if $LINEITEM_CUSTOM_FIELD['hideInitialDisplay'] eq 'true'} class="hide" {/if}>
							<select {if $LINEITEM_CUSTOM_FIELD['editable'] eq 0} readonly style="pointer-events: none;background-color:#eeeeee;min-width: 100px;" {/if}
							data-rule-required=true style="min-width: 150px;" id="{${$fieldName}}" 
							class="select2-container {if $row_no neq 0}select2{/if} inputElement picklistfield {$fieldName}" 
			...
			{elseif $LINEITEM_CUSTOM_FIELD['uitype'] eq '69'}
				<div id="{$fieldName}DivCla" {if $LINEITEM_CUSTOM_FIELD['hideInitialDisplay'] eq 'true'} class="hide" {/if}>
					<input type="file" class="inputElement {if $MODULE eq 'Products' or $MODULE eq 'HelpDesk' or $MODULE eq 'ServiceReports'}multi max-6{/if} {if $FIELD_MODEL->get('fieldvalue') and $FIELD_INFO["mandatory"] eq true} ignore-validation {/if}"
					name="{${$fieldName}}[]"
					{if !empty($SPECIAL_VALIDATOR)}data-validator="{Zend_Json::encode($SPECIAL_VALIDATOR)}" {/if}
					{if $FIELD_INFO["mandatory"] eq true} data-rule-required="true" {/if} />
				</div>
