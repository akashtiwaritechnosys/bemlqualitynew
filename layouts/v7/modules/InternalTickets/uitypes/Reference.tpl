...
<div class="referencefield-wrapper {if $FIELD_VALUE neq 0} selected {/if}">
    ...
    <div class="input-group">
        ...
        <input id="{$FIELD_NAME}_display" name="{$FIELD_MODEL->getFieldName()}_display" data-fieldname="{$FIELD_MODEL->getFieldName()}" data-fieldtype="reference" type="text" 
            ...
            {/if}
            {if $FIELD_MODEL->get('name') eq 'service_engineerid'} disabled {/if} />

        <a href="#" class="clearReferenceSelection {if $FIELD_MODEL->get('name') eq 'service_engineerid'} hide {/if}  {if $FIELD_VALUE eq 0}hide{/if}"> x </a>
            <span class="input-group-addon relatedPopup cursorPointer {if $FIELD_MODEL->get('name') eq 'service_engineerid'} hide {/if}" title="{vtranslate('LBL_SELECT', $MODULE)}">
        ...
        {if (($smarty.request.view eq 'Edit') or ($MODULE_NAME eq 'Webforms')) && !in_array($REFERENCE_LIST[0],$QUICKCREATE_RESTRICTED_MODULES)}
            <span class="input-group-addon createReferenceRecord cursorPointer clearfix {if $FIELD_MODEL->get('name') eq 'service_engineerid'} hide {/if}" title="{vtranslate('LBL_CREATE', $MODULE)}">
