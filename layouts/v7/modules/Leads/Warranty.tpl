{strip}
	...
	<table id="data_table">
		...
		<tr>
			<th class="NoBackGround" rowspan="1">Model</th>
						...
						{foreach item=INNER_VALUE2 key=INNER_KEY2 from=$INNER_VALUE1}
							...
							{else}
								<th rowspan="1" class="NoBackGround" class="NoBackGround">
									<a target="_blank"
										href='index.php?module=Equipment&view=List&app=INVENTORY&search_params=[[["{$ColumnColIg}","e","{$INNER_KEY2}"],["{$rowColIg}","e","{$INNER_KEY}"],["equip_category","e","S"],["eq_run_war_st","e","Under Warranty"]]]'>
										{$INNER_VALUE2}
									</a>
								</th>
							{/if}
