...
$( document ).ready(function() {
 ...
 {include file="exports.tpl"|vtemplate_path:$MODULE TITLE=$HEADER_TITLE}
        ...
        <table id="data_table">
            ...
            <tr>
                <th class="NoBackGround" rowspan="1">Region</th>
                {foreach item=INNER_VALUE1 key=INNER_KEY1 from=$PickListValues}
                    <th rowspan="1" class="NoBackGround" class="NoBackGround"><b>{$INNER_VALUE1}</b></th>
                {/foreach}
                <th class="NoBackGround" colspan="1"> <b> Grand Total </b> </th>
            </tr>
                        ...
                        {foreach item=INNER_VALUE1 key=INNER_KEY1 from=$NAME_FIELD}
                            ...
                            {else}
                                <th rowspan="1" class="NoBackGround" class="NoBackGround">
                                	{if $KEY_FIELD eq 'Grand Total'}
                                		{$INNER_VALUE1}
                                	{else}
                                		<a href="index.php?module=Equipment&view=List&orderby=&sortorder=&app=INVENTORY&search_params={$ListViewURL[$KEY_FIELD][$INNER_KEY1]}" target="_blank">{$INNER_VALUE1}</a>
                                	{/if}
                                </th>
                            {/if}
                        {/foreach}
