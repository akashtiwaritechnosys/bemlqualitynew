...
$( document ).ready(function() {
 ...
 {include file="exports.tpl"|vtemplate_path:$MODULE TITLE=$HEADER_TITLE}
        ...
        <table id="data_table">
            ...
            <tr>
                <th class="NoBackGround" rowspan="1">
                    <b style="font-weight: 900">Model</b>
                </th>
                {foreach item=INNER_VALUE1 key=INNER_KEY100 from=$PickListValues}
                    <th rowspan="1" class="NoBackGround">
                        <b style="font-weight: 900">{$INNER_VALUE1}</b>
                    </th>
                {/foreach}
                <th class="NoBackGround" colspan="1">
                    <b style="font-weight: 900"> Grand Total </b>
                </th>
            </tr>
            {foreach item=NAME_FIELD key=KEY_FIELD from=$ResultArray}
                <tr>
                    <th class="NoBackGround">{$KEY_FIELD}</th>
                    {foreach item=INNER_VALUE1 key=INNER_KEY1 from=$NAME_FIELD}
                        {foreach item=INNER_VALUE2 key=INNER_KEY2 from=$INNER_VALUE1}
                            ...
                            {else}
                                {if $INNER_KEY2 eq 'Sub Total' or $INNER_KEY2 eq 'Grand Total'}
                                    <th rowspan="1" style="font-weight: 900" class="NoBackGround" class="NoBackGround">
                                        <a target="_blank"
                                            href='index.php?module=Equipment&view=List&app=INVENTORY&search_params=[[["equip_model","e","{$KEY_FIELD}"],["eq_run_war_st","e","Contract"],["equip_category","e","S"]]]'>
                                            {$INNER_VALUE2}
                                        </a>
                                    </th>
                                {elseif $KEY_FIELD eq 'Grand Total'}
                                    <th rowspan="1" style="font-weight: 900" class="NoBackGround" class="NoBackGround">
                                        <a target="_blank"
                                            href='index.php?module=Equipment&view=List&app=INVENTORY&search_params=[[["eq_type_of_conrt","e","{$INNER_KEY2}"],["eq_run_war_st","e","Contract"],["equip_category","e","S"]]]'>
                                            {$INNER_VALUE2}
                                        </a>
                                    </th>
                                {else}
                                    <th rowspan="1" class="NoBackGround" class="NoBackGround">
                                        <a target="_blank"
                                            href='index.php?module=Equipment&view=List&app=INVENTORY&search_params=[[["equip_model","e","{$KEY_FIELD}"],["eq_type_of_conrt","e","{$INNER_KEY2}"],["eq_run_war_st","e","Contract"],["equip_category","e","S"]]]'>
                                            {$INNER_VALUE2}
                                        </a>
                                    </th>
                                {/if}
