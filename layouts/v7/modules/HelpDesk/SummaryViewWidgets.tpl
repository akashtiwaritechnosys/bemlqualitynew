    ...
    <div class="middle-block col-lg-7">
        ...
        <div class="summaryWidgetContainer">
            <div class="widgetContainer_comments">
                <table id="">
                    {foreach item=NAME_FIELD1 key=KEY_FIELD from=$STATUS_CHANGE_HISTORY}
                        <tr>
                            {foreach item=INNER_VALUE1 key=INNER_KEY1 from=$NAME_FIELD1}
                                {if $INNER_KEY1 eq 'duration_seconds'}
                                    <th>{Vtiger_Functions::displayTimeSecondsReadbleFormat($INNER_VALUE1)}</th>
                                {elseif $INNER_KEY1 eq 'changed_at'}
                                    <th>{Vtiger_Functions::getDisplayDateTimeValueTemplate($INNER_VALUE1)}</th>
                                {else}
                                    <th>{$INNER_VALUE1}</th>
                                {/if}
                            {/foreach}
