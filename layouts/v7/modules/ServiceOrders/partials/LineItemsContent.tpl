{strip}
			...
			{if $data.$productDeleted}
				<div class="row-fluid deletedItem redColor">
					{if empty($LINE_ITEM_DETAIL["productName$INDEX"])}
						{vtranslate('Manualy Added Part Number',$MODULE)}
					{else}
						{vtranslate('This Part Is Added',$MODULE)} {vtranslate('Manualy',$MODULE)}
					{/if}
				</div>
			{/if}
		</td>
	...
	{* {if $data.$collect_immidiately eq 1} readonly {/if} *}
		<input id="{$qty}" name="{$qty}" type="text" class="qty smallInputBox inputElement"
			  data-rule-positive=true data-rule-greater_than_zero=true value="{if !empty($data.$qty)}{$data.$qty}{else}1{/if}"
