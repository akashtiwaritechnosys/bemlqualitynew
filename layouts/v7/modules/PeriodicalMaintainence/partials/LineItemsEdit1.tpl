{strip}
	...
	<div name='editContent'>
		...
		{assign var=ITEMBLOCKNAMEANOTHER value='Periodic Schedule'}
		{if $BLOCK_FIELDS|php7_count gt 0 or 1 == 1}
			<div class='fieldBlockContainer' id="ShortagefieldBlockContainer" >
				...
				<div class="lineitemTableContainerAno">
					...
					{* <td><strong>{vtranslate('LBL_TOOLS',$MODULE)}</strong></td> *}
						...
						<tr id="row" class="lineItemCloneCopy" data-row-num="0">
							{include file="partials/LineItemsContent1.tpl"|@vtemplate_path:'PeriodicalMaintainence' tabletdhiderVal= true row_no=0 data=[] IGNORE_UI_REGISTRATION=true}
						</tr>
					{if php7_count($RELATED_PRODUCTS_OTHER1) gt 0}
							{foreach key=row_no item=data from=$RELATED_PRODUCTS_OTHER1}
								<tr id="row{$row_no}" data-row-num="{$row_no}" class="lineItemRow" >
									{include file="partials/LineItemsContent1.tpl"|@vtemplate_path:'PeriodicalMaintainence' row_no=$row_no data=$data}
								</tr>
							{/foreach}
						{/if}
						</tbody>
