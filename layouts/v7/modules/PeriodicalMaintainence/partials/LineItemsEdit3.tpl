{strip}
{foreach item=LINEITEM_CUSTOM_FIELD from=$LINEITEM_CUSTOM_FIELDS_OTHER3}
...
<input type="hidden" name="totalProductCount3" id="totalProductCount3" value="{$row_no}" />
...
<div name='editContent'>
	...
	{assign var=ITEMBLOCKNAMEANOTHER value='Engineers Details'}
	{if $BLOCK_FIELDS|php7_count gt 0 or 1 == 1}
		<div class='fieldBlockContainer' >
			...
			<div class="lineitemTableContainer">
				<table class="table table-bordered" id="lineItemTab3">
				<thead><td><strong>{vtranslate('LBL_TOOLS',$MODULE)}</strong></td>
						{foreach key=LINEITEM_CUSTOM_FIELDKEY item=LINEITEM_CUSTOM_FIELD from=$LINEITEM_CUSTOM_FIELDS3}
					...
					<tbody id="appendRows">
					<tr id="row3r0" class="hide lineItemCloneCopy" data-row-num="0">
						{include file="partials/LineItemsContent3.tpl"|@vtemplate_path:'PeriodicalMaintainence' row_no=0 data=[] IGNORE_UI_REGISTRATION=true}
					</tr>
				{if php7_count($RELATED_PRODUCTS_OTHER3) gt 0}
						{foreach key=row_no item=data from=$RELATED_PRODUCTS_OTHER3}
							<tr id="row{$row_no}" data-row-num="{$row_no}" class="lineItemRow" >
								{include file="partials/LineItemsContent3.tpl"|@vtemplate_path:'PeriodicalMaintainence' row_no=$row_no data=$data}
							</tr>
						{/foreach}
					{/if}
					</tbody>
	...
	{* <div>
			<div class="btn-toolbar">
				<span class="btn-group">
					<button type="button" class="btn btn-default" id="addProduct3" data-module-name="Contacts" >
						<i class="fa fa-plus"></i>&nbsp;&nbsp;<strong>{vtranslate('Add Engineer',$MODULE)}</strong>
					</button>
				</span>
			</div>
	</div> *}