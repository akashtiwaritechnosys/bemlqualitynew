{strip}
    ...
    <div name='editContent'>
        ...
        {foreach key=BLOCK_LABEL item=BLOCK_FIELDS from=$RECORD_STRUCTURE name=blockIterator}
            ...
            {if $BLOCK_LABEL eq 'Periodic_Schedule'}
                {include file="partials/LineItemsEdit2.tpl"|@vtemplate_path:'PeriodicalMaintainence'}
                {continue}
            {/if}
            {if $BLOCK_LABEL eq 'Engineers_List'}
                {include file="partials/LineItemsEdit3.tpl"|@vtemplate_path:'PeriodicalMaintainence'}
                {continue}
            {/if}
            {if $BLOCK_LABEL eq 'Periodic_Schedule_Block_Date'}
                {include file="partials/LineItemsEdit1.tpl"|@vtemplate_path:'PeriodicalMaintainence'}
                {continue}
            {/if}
                        ...
                        {if ($BLOCK_LABEL eq 'LBL_ADDRESS_INFORMATION') and ($MODULE neq 'PurchaseOrder')}
                            <tr>
                                ...
                                <td class="fieldValue" name="copyAddress1">
                                    <div class="radio">
                                        <label>
                                            <input type="radio" name="copyAddressFromRight" class="accountAddress"
                                                data-copy-address="billing" checked="checked">
                                    ...
                                    <div class="radio">
                                        <label>
                                            {if $MODULE eq 'Quotes'}
                                                <input type="radio" name="copyAddressFromRight" class="contactAddress"
                                                    data-copy-address="billing" checked="checked">
                                    ...
                                    <div class="radio" name="togglingAddressContainerRight">
                                        <label>
                                            <input type="radio" name="copyAddressFromRight" class="shippingAddress"
                                                data-target="shipping" checked="checked">
                                            &nbsp;{vtranslate('Shipping Address', $MODULE)}
                                        </label>
                                    </div>
                                    <div class="radio hide" name="togglingAddressContainerLeft">
                                        <label>
                                            <input type="radio" name="copyAddressFromRight" class="billingAddress" data-target="billing"
                                                checked="checked">
                                ...
                                <td class="fieldValue" name="copyAddress2">
                                    <div class="radio">
                                        <label>
                                            <input type="radio" name="copyAddressFromLeft" class="accountAddress"
                                                data-copy-address="shipping" checked="checked">
                                            &nbsp;{vtranslate('SINGLE_Accounts', $MODULE)}
                                        </label>
                                    </div>
                                    <div class="radio">
                                        <label>
                                            {if $MODULE eq 'Quotes'}
                                                <input type="radio" name="copyAddressFromLeft" class="contactAddress"
                                                    data-copy-address="shipping" checked="checked">
                                                &nbsp;{vtranslate('Related To', $MODULE)}
                                            {else}
                                                <input type="radio" name="copyAddressFromLeft" class="contactAddress"
                                    ...
                                    <div class="radio" name="togglingAddressContainerLeft">
                                        <label>
                                            <input type="radio" name="copyAddressFromLeft" class="billingAddress" data-target="billing"
                                                checked="checked">
                                            &nbsp;{vtranslate('Billing Address', $MODULE)}
                                        </label>
                                    </div>
                                    <div class="radio hide" name="togglingAddressContainerRight">
                                        <label>
                                            <input type="radio" name="copyAddressFromLeft" class="shippingAddress"
                                                data-target="shipping" checked="checked">
                                            &nbsp;{vtranslate('Shipping Address', $MODULE)}
                                        </label>
                                    </div>
                        ...
                        <tr>
                            ...
                            {foreach key=FIELD_NAME item=FIELD_MODEL from=$BLOCK_FIELDS name=blockfields}
                                ...
                                {if $FIELD_MODEL->isEditable() eq true}
                                    ...
                                    <td {if in_array($FIELD_MODEL->get('uitype'),array('19','69')) || $FIELD_NAME eq 'description'}
                                        class="fieldValue fieldValueWidth80" colspan="3" {assign var=COUNTER value=$COUNTER+1} {else}
                                        class="fieldValue" {/if}>
