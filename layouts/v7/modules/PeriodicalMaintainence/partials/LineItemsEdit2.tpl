{strip}
	{foreach item=LINEITEM_CUSTOM_FIELD from=$LINEITEM_CUSTOM_FIELDS_OTHER2}
	...
	{assign var="FINAL" value=$RELATED_PRODUCTS_OTHER2.1.final_details}
	...
	<input type="hidden" disabled="disabled" id="fildNamesOfCustPickFieldsInfoOther"
		value={ZEND_JSON::encode($LINEITEM_CUSTOM_OTHER_PICK_FIELDS2)}>
	<table id="lastPeriodialTable" class="table table-bordered fieldBlockContainer hide">
		<input type="hidden" disabled="disabled" id="fildNamesOfCustPickFieldsInfo"
			value={ZEND_JSON::encode($LINEITEM_CUSTOM_PICK_FIELDS2)}>
		<tbody></tbody>
	</table>
	<div name='editContent'>
		...
		{assign var=ITEMBLOCKNAMEANOTHER value='Asset Details'}
				...
				<div class="lineitemTableContainer">
					<table class="table table-bordered" id="lineItemTab">
						<thead>
							...
							{foreach key=LINEITEM_CUSTOM_FIELDKEY item=LINEITEM_CUSTOM_FIELD from=$LINEITEM_CUSTOM_FIELDS2}
						...
						<tbody id="appendRows">
							<tr id="row0" class="hide lineItemCloneCopy" data-row-num="0">
								{include file="partials/LineItemsContent2.tpl"|@vtemplate_path:'PeriodicalMaintainence' row_no=0 data=[] IGNORE_UI_REGISTRATION=true}
							</tr>
							{if php7_count($RELATED_PRODUCTS_OTHER2) gt 0}
								{foreach key=row_no item=data from=$RELATED_PRODUCTS_OTHER2}
									<tr id="row{$row_no}" data-row-num="{$row_no}" class="lineItemRow">
										{include file="partials/LineItemsContent2.tpl"|@vtemplate_path:'PeriodicalMaintainence' row_no=$row_no data=$data}
									</tr>
								{/foreach}
							{/if}
						</tbody>
	...
	<div>
		<div class="btn-toolbar">
			<span class="btn-group">
				<button type="button" class="btn btn-default" id="addProduct" data-module-name="Contacts">
					<i class="fa fa-plus"></i>&nbsp;&nbsp;<strong>{vtranslate('Add Asset',$MODULE)}</strong>
				</button>
			</span>
		</div>
	</div>
