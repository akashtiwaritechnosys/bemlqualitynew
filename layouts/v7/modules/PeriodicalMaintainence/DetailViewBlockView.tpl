...
{strip}
	...
	{foreach key=BLOCK_LABEL_KEY item=FIELD_MODEL_LIST from=$RECORD_STRUCTURE}
		...
		{if $BLOCK eq null or $FIELD_MODEL_LIST|php7_count lte 0}
		...
		<div class="block block_{$BLOCK_LABEL_KEY}" data-block="{$BLOCK_LABEL_KEY}"
							...
							{foreach item=FIELD_MODEL key=FIELD_NAME from=$FIELD_MODEL_LIST}
									...
									<td class="fieldValue {$WIDTHTYPE}">
										<ul id="imageContainer" class="imageSlider">
											{* {if empty($IMAGE_DETAILS)}
												...
												{/if} *}
											{if $MODULE_NAME eq 'PeriodicalMaintainence' ||  $MODULE_NAME eq 'ServiceReports'}
													...
													{if $FIELD_MODEL->getName() eq $IMAGE_INFO.fieldNameFromDB}
														{if ( strpos($IMAGE_INFO.name , '.png') >= 1 || strpos($IMAGE_INFO.name , '.jpeg') >= 1 || strpos($IMAGE_INFO.name , '.jpg') >= 1)  }
															<li>
																<span>
																	<a
																		onclick="Vtiger_Header_Js.fileView({$RECORDID},{$IMAGE_INFO.id});">{$IMAGE_INFO.name}</a>
																</span>
															</li>
														{else}
															<li>
											...
											{else}
												{foreach key=ITER item=IMAGE_INFO from=$IMAGE_DETAILS}
													{if !empty($IMAGE_INFO.url) && !empty({$IMAGE_INFO.orgname}) }
														{if ( strpos($IMAGE_INFO.name , '.png') >= 1 || strpos($IMAGE_INFO.name , '.jpeg') >= 1 || strpos($IMAGE_INFO.name , '.jpg') >= 1)  }
															<li><img src="{$IMAGE_INFO.url}" title="{$IMAGE_INFO.orgname}" width="400"
																	height="300" /><span><a href="{$IMAGE_INFO.url}">{$IMAGE_INFO.name}</a></span></li>
														{else}
															<li><img style="display:none" data-image-id="{$IMAGE_INFO.id}" target="_blank"
																	title="{$IMAGE_INFO.name}" /><span><a
																		href="{$IMAGE_INFO.url}">{$IMAGE_INFO.name}</a></span></li>
														{/if}
													{/if}
												{/foreach}
											{/if}
								...
								{else}
									...
									<td class="fieldValue {$WIDTHTYPE}"
										...
										<span class="value" id="{$FIELD_MODEL->getName()}"
			...
			{if $BLOCK_LABEL_KEY eq 'Payment_Scheduling'}
				{include file="LineItemsDetailShortDam1.tpl"|@vtemplate_path:'SalesOrder'}
			{/if}
		</div>
